...
running native Elastic EP worker or communicator reconfiguration. CAMP2P owns
that reconfiguration; vLLM only needs its frontend routing and DP coordinator
engine counts reduced for the startup PoC.
"""
...
# groups on Ascend.
# Patch functionality: for the CAMP2P startup PoC only, retire tail Attention
# engine identities from request routing and notify DPCoordinator of the new
# count. The native implementation remains unchanged for every other setup.
...
async def scale_elastic_ep(
    ...
    # ### PATCH START: CAMP2P routing-only tail retirement
    ...
    # ### PATCH END: CAMP2P routing-only tail retirement
